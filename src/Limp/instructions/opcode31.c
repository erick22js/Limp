...
// RET
void LIIsaDtb_opcode31_func00(LCpu *m_cpu){
	Uint32 addr = LCpu_pop(m_cpu);
	LCpu_jumpAbs(m_cpu, addr);
}

// IRET
void LIIsaDtb_opcode31_func01(LCpu *m_cpu){
	// Protected Mode Only
...
// ENTER
void LIIsaDtb_opcode31_func02(LCpu *m_cpu){
	Uint32 stacksize;
	...
	LCpu_push(m_cpu, m_cpu->regs.n.fp);
	
	m_cpu->regs.n.sp -= 4;
	m_cpu->regs.n.fp = m_cpu->regs.n.sp;
	m_cpu->regs.n.sp -= stacksize;
	
	LCpu_push(m_cpu, m_cpu->regs.n.fp);
}

// LEAVE
void LIIsaDtb_opcode31_func03(LCpu *m_cpu){
	m_cpu->regs.n.sp = LCpu_pop(m_cpu);
	
	m_cpu->regs.n.sp += 4;
	m_cpu->regs.n.fp = LCpu_pop(m_cpu);
}

// INT
void LIIsaDtb_opcode31_func04(LCpu *m_cpu){
...
extern void (*LIIsaDtb_opcode31[64])(LCpu *m_cpu) = {
	[0x00] = LIIsaDtb_opcode31_func00,
	[0x01] = LIIsaDtb_opcode31_func01,
	[0x02] = LIIsaDtb_opcode31_func02,
	[0x03] = LIIsaDtb_opcode31_func03,
	[0x04] = LIIsaDtb_opcode31_func04,
