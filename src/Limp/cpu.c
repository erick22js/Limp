...
Bool LCpu_requestInterruption(LCpu *m_cpu, LIPInterruption intr){ // Returns Bool state => TRUE if the interruption was successfull did
	...
	//Log("Cpu interruption from vector: 0x%x\n", intr.selector);
	Uint32 addr = LICPU_readBus32(m_cpu, (m_cpu->sregs.it + (intr.selector*4)));
	LICpu_int_call(m_cpu, addr);
	m_cpu->sregs.est = (m_cpu->sregs.est&0x00FFFFFF)|(intr.selector<<24);
